  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between w-full px-4 py-2 md:py-3">
        ...
        >
          {/* Show X if menu open, hamburger otherwise */}
          {showMobileMenu ? "✕" : "☰"}
        </button>
      ...
      <div className="w-full border-t border-black/10 bg-transparent">
        ...
        {showMobileMenu && (
          <div className="md:hidden bg-white/90 backdrop-blur-xl px-6 py-4 flex flex-col gap-4 shadow-md border-b border-black/10 animate-fadeIn">
            <Link href="/products" className="py-2 border-b border-gray-100" onClick={() => setShowMobileMenu(false)}>All</Link>
            <Link href="/products/search/dress" className="py-2 border-b border-gray-100" onClick={() => setShowMobileMenu(false)}>Dresses</Link>
            <Link href="/products/search/top" className="py-2 border-b border-gray-100" onClick={() => setShowMobileMenu(false)}>Tops</Link>
            <Link href="/products/search/bottom" className="py-2 border-b border-gray-100" onClick={() => setShowMobileMenu(false)}>Bottoms</Link>
            <Link href="/products/search/set" className="py-2 border-b border-gray-100" onClick={() => setShowMobileMenu(false)}>Sets</Link>
            <Link href="/products/search/outerwear" className="py-2 border-b border-gray-100" onClick={() => setShowMobileMenu(false)}>Outerwear</Link>
            <Link href="/products/search/accessories" className="py-2 border-b border-gray-100" onClick={() => setShowMobileMenu(false)}>Accessories</Link>
            <Link href="/products/search/pattern" className="py-2 border-b border-gray-100" onClick={() => setShowMobileMenu(false)}>Patterns DIY</Link>
            <Link href="/custom-order" className="py-2 border-b border-gray-100" onClick={() => setShowMobileMenu(false)}>Custom Order</Link>
            {session?.user?.role === "admin" && (
              <div className="flex flex-col gap-2 text-red-600 font-semibold mt-2">
                <Link href="/admin" className="hover:underline" onClick={() => setShowMobileMenu(false)}>Admin</Link>
                <Link href="/admin/orders" className="hover:underline" onClick={() => setShowMobileMenu(false)}>Orders</Link>
              </div>
            )}
          </div>
        )}
